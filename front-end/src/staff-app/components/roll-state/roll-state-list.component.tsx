    ...
    <S.ListContainer>
      {rollStateList.map((s: any, i: any) => {
        if (s.type === "all") {
          return (
            <S.ListItem key={i}>
              <FontAwesomeIcon icon="users" size="sm" style={{ cursor: "pointer" }} onClick={() => dispatch({ type: "FILTER_STUDENT_ROLE", payload: s.type })} />
        ...
        return (
          <S.ListItem key={i}>
            <RollStateIcon type={s.type} size={size} onClick={() => dispatch({ type: "FILTER_STUDENT_ROLE", payload: s.type })
              } />
